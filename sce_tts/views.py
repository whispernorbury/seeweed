...
def normalize_multiline_text(long_text, symbols):
    texts = split_text(long_text)
    normalized_texts = [normalize_text(text, symbols).strip() for text in texts]
    result_texts = ''
    for text in normalized_texts:
        result_texts += text
    return [result_texts,]
# [text for text in normalized_texts if len(text) > 0]